...
export async function createManualReceipt(
  ...
  // Nachlesen statt RETURNING: so kommt die Zeile mit Abrechnung zurück wie
  // jede andere Antwort, und der Fall "gibt es schon" braucht keinen eigenen Weg.
  return getReceipt(userId, input.id);
}
...
export async function updateReceipt(
  ...
  const outcome = await db.transaction(async tx => {
    ...
    // Die Sperre bleibt im WHERE und nicht in einer Vorabprüfung – siehe
    // receiptIsEditable(). Die Zeile ist oben zwar gesperrt, die Abrechnung
    // aber nicht.
    const [row] = await tx
      .update(receipts)
    ...
    return row ? ('updated' as const) : ('locked' as const);
  });

  if (outcome === 'missing') return null;
  if (outcome === 'locked') throw new ReceiptLockedError();
