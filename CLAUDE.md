...
- `channels/receipt-review-card.ts` + `channels/receipt-card-handlers.ts` + `channels/receipt-review-session.ts` — the presentation is an **Adaptive Card** (Confirm & save / Adjust / Cancel). A card click is not a message in Teams: the adapter sees `value.actionId` and routes it to `chat.processAction`, so the message handlers never see it. Own handlers therefore hang off `agent.getChannels().sdk` (`onAction` / `onModalSubmit`), registered once at startup in `index.ts` — not on the first receipt, or a card posted before a deploy would be dead after it. Mastra registers its own catch-all `onAction` for tool approvals and ignores foreign ids; handlers are additive, hence the `receipt-review:` prefix, with the runId in the action id so a stale card cannot confirm a newer receipt. `receipt-review-session.ts` is the shared middle of both paths (resume + report): whether a booking is written must not depend on clicking versus typing. A card cannot carry inputs (`CardChild` has none) — so "Adjust" is a button with `actionType: 'modal'` (→ `msteams: task/fetch`) opening a Teams dialog for exactly the three confirmable fields (`REVIEW_FIELDS`): date, currency, total. VAT is deliberately not confirmed — it is still extracted and stored, but not asked for. Dialog input goes through `applyReviewEdits()` in `receipts/candidate.ts` — the same parsers as extraction; empty means null, unreadable re-opens the dialog with the message instead of silently storing null. The dialog submit *is* the confirmation (`resumeData.kind === 'edit'`). Free text in the thread still works and re-presents the card.
...
- `receipts/file-format.ts` — magic-byte detection and normalization. JPEG/PNG/WebP/GIF and PDF are stored as-is; HEIC/HEIF, AVIF, TIFF, BMP are converted to JPEG once at intake (sharp; `heic-decode` for HEVC, which prebuilt libvips cannot decode; `bmp-js` for BMP), animated GIF → first frame PNG. SVG is rejected on purpose.
- `receipts/upload-store.ts` — `storeUpload(bytes, filename)` sniffs + normalizes via `file-format.ts`, stored types (`.jpg/.png/.webp/.gif/.pdf`), `MAX_UPLOAD_BYTES` (15 MB), and `UPLOAD_ID_PATTERN`. The strict `<uuid><ext>` pattern *is* the path-traversal defense — don't loosen it.
...
- Stored file types: `STORED_TYPES` (`file-format.ts`), `UPLOAD_ID_PATTERN` (`upload-store.ts`) and `MIME_BY_EXT` (both `receipt-extraction-workflow.ts` and `server/receipt-routes.ts`). A PDF must go to the model as a **file** part, never an image part — providers reject a PDF image part. The frontend's `ReceiptImage` embeds PDFs instead of using `<img>`. `sharp`, `heic-decode` and `bmp-js` must stay real dependencies (native module / Wasm); `mastra build` installs them into `.mastra/output/node_modules`.
...
- Teams reports inline-pasted images as `image/*`, uploaded PDFs as `application/pdf` (type `file`), and unknown extensions (`.heic`, `.tif`, `.bmp`) as `application/octet-stream`, so the real format is sniffed from magic bytes in `detectReceiptFormat()` rather than trusted from `contentType`.
- `MASTRA_MODEL` must read both images and PDFs.

## Conventions

- **Everything the bot says in Teams is English** — card, dialog, thread messages, workflow result `message`, `teams-agent` / `receipt-correction-agent` instructions and the `issues` the models write — regardless of the user's language (input in any language is fine). German error texts from the API/workflow go to the log only; the Teams path words its own English message. Otherwise, comments and user-facing strings (error messages, tool descriptions) are **German**. `receipt-agent.ts` / `receipt-extraction-workflow.ts` internals are English. Match the file you're editing. In the frontend, comments, commit-relevant prose and error logs are German, but the **UI text is English** — it comes verbatim from the mockup. Numbers and dates stay Swiss (de-CH).
