...
export const INPUT_CLASS =
  "bg-panel text-ink h-11 w-full rounded-[11px] border px-3 text-sm placeholder:text-ink-3";
        ...
        {manual ? (
          <EditRow label="reason" error={error("reason")}>
            <textarea
              ...
              required
              maxLength={1000}
              className={`bg-panel text-ink min-h-[80px] w-full resize-y rounded-[11px] border p-3 text-sm leading-[1.55] ${borderFor(error("reason"))}`}
            />
          </EditRow>
        ) : (
          <>
