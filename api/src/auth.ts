...
//
// Zwei Wege hinein, bewusst asymmetrisch:
//
//   Service-Token  ->  der Mastra-Agent. Er darf das Subject frei setzen, weil
//                      er es aus einem signierten Bot-Framework-Payload nimmt
//                      (message.author.userId) und nie aus Modell-Output. Das
//                      ist der "Superuser"-Weg: ein Prozess, dem wir vertrauen,
//                      handelt im Namen eines Nutzers.
//
//   Entra-JWT      ->  ein angemeldeter Mensch. Das Subject kommt aus `oid` im
//                      Token und ist NICHT setzbar: ein mitgeschickter
//                      X-Subject-User-Header wird ignoriert, nicht gelesen.
//                      Sonst waere die Mandantentrennung eine Bitte.
//
// Beide muenden in denselben Wert: eine Teams-userId, die jede
...
export type Actor = { kind: 'service'; name: string } | { kind: 'user'; aadObjectId: string };
...
async function resolveUserSubject(token: string): Promise<AuthState> {
  ...
  // Die Bruecke aus Phase 1: Entra-Identitaet -> Teams-userId, an der die
  // Belege haengen. Wer sich anmeldet, aber nie ueber Teams erfasst hat, hat
  // hier keine Zeile - und sieht dann nichts. Das ist richtig so: ein leerer
  // Scope heisst "darf nichts sehen", nicht "darf alles sehen".
  ...
  return { actor: { kind: 'user', aadObjectId }, subject: teamsUserIds[0]! };
}
...
export const authenticate: MiddlewareHandler<{ Variables: { auth: AuthState } }> = async (
  ...
  if (timingSafeEqual(token, SERVICE_TOKEN)) {
    const subject = c.req.header('x-subject-user')?.trim();
    if (!subject) {
      throw new HTTPException(400, {
        message: 'Service-Aufruf ohne X-Subject-User: unklar, fuer wen gearbeitet werden soll.',
      });
    }
    c.set('auth', { actor: { kind: 'service', name: 'mastra-agent' }, subject });
    return next();
  }
